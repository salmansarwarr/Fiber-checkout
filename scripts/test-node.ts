...
const CURRENCY = (process.env.FIBER_CURRENCY ?? "Fibt") as
    | "Fibt"
    | "Fibb"
    | "Fibd";
...
async function test(name: string, fn: () => Promise<void>) {
    ...
    try {
        ...
        if (FiberError.is(err)) {
            ...
            if (err.rpcCode !== undefined)
                console.error(`    RPC code: ${err.rpcCode}`);
        } else {
            console.error(`    ${err}`);
        }
        failed++;
...
await test("new_invoice — creates CKB invoice (100 shannon)", async () => {
    ...
    if (!result.invoice.data.payment_hash)
...
await test("new_invoice — creates hold invoice (payment_hash only)", async () => {
    ...
    // Note: the @nervosnetwork/fiber-js SDK type for NewInvoiceParams doesn't include
    // payment_hash (hold invoice pattern), but the RPC endpoint itself accepts it.
    // We use a type assertion here for this edge-case test.
    const params = {
        amount: "0x186a0", // 100_000 shannon
        currency: CURRENCY,
        payment_hash: fakeHash,
        description: "hold invoice test",
    } as unknown as NewInvoiceParams;
...
await test("get_invoice — retrieves invoice by payment_hash", async () => {
    if (!paymentHash)
        throw new Error("Skipped: new_invoice failed, no payment_hash");

    const result = await client.getInvoice({ payment_hash: paymentHash });

    if (!result.status) throw new Error("Missing status");
    if (result.status !== "Open")
        throw new Error(`Expected status Open, got ${result.status}`);
...
await test("get_invoice — unknown hash returns RPC_ERROR (not a crash)", async () => {
    ...
    try {
        ...
        if (err.code !== "RPC_ERROR")
            throw new Error(`Expected RPC_ERROR, got ${err.code}`);
...
if (failed > 0) {
    ...
    console.log(
        `is running with dangerouslyAllowDirectRpc: true (already set here).`,
    );
    console.log(
