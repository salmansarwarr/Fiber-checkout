...
function statusColors(status: CheckoutStatus): React.CSSProperties {
  switch (status) {
    case "pending":    return { background: "#f5f5f5", color: "#555" };
    case "processing": return { background: "#fff8e1", color: "#b45309" };
    case "success":    return { background: "#e6f9f0", color: "#15803d" };
    case "failed":     return { background: "#fef2f2", color: "#b91c1c" };
    case "expired":    return { background: "#fef2f2", color: "#b91c1c" };
...
export function FiberCheckout({
  ...
  // ── Derived state ───────────────────────────────────────────────────────────
  const isTerminal = status === "success" || status === "failed" || status === "expired";
  ...
  // ── Expiry countdown ────────────────────────────────────────────────────────
  const expiryDate = expiresAt ? new Date(expiresAt).toLocaleTimeString() : null;
  ...
  return (
    <div style={styles.container}>
      ...
      {invoiceAddress && status === "pending" && (
        <button style={styles.copyButton} onClick={handleCopy} type="button">
          Copy invoice
        </button>
      )}
      ...
      {(isTerminal || invoiceError) && status !== "success" && (
        <button style={styles.retryButton} onClick={regenerate} type="button">
