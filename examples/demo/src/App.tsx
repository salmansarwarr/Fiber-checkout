...
import { useFiberInvoice, useFiberPayment, formatAmount, ckbToShannonHex } from "fiber-checkout";
...
const INVOICE_NODE_URL = import.meta.env.VITE_FIBER_INVOICE_NODE_URL ?? NODE_URL;
    ...
    <div className={styles.layout}>
      ...
      <aside className={styles.sidebar}>
        ...
        <p className={styles.sidebarDesc}>
          Drop-in React component for Fiber Network payments.
          Select an asset and amount to generate a testnet invoice.
        </p>
          ...
          <div className={styles.assetGroup}>
            {ASSETS.map((a) => (
              <button
                ...
                onClick={() => { setAsset(a.id); setSuccessHash(null); }}
                type="button"
              >
                {a.label}
              </button>
        ...
        <div className={styles.field}>
          ...
          <div className={styles.presets}>
            {PRESETS.map((p) => (
              <button
                ...
                onClick={() => { setCkbAmount(p); setSuccessHash(null); regenerate(); }}
                type="button"
              >
                {p}
              </button>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (v > 0) { setCkbAmount(v); setSuccessHash(null); }
            }}
          />
        </div>
...
function CheckoutView({
  invoiceAddress, invoiceLoading, status, error,
  expiresAt, ckbAmount, asset, amountHex, copied,
  isTerminal, paying, onCopy, onPay, onRegenerate,
}: {
  ...
  return (
    <>
      <div className={styles.cardHeader}>
        <div className={styles.amount}>{ckbAmount} <span className={styles.assetLabel}>{asset}</span></div>
        <div className={styles.amountHex}>{amountHex}</div>
      </div>

      {/* QR */}
      <div className={`${styles.qrArea} ${isTerminal ? styles.qrAreaDimmed : ""}`}>
        {invoiceLoading ? (
          <div className={styles.qrSkeleton}>
            <span className={styles.skeletonText}>Generating invoice…</span>
          </div>
          ...
          <div className={styles.qrSkeleton}>
            <span className={styles.skeletonText}>{isTerminal ? status : "—"}</span>
          </div>
        )}
      </div>
      ...
      <div className={styles.actions}>
        ...
        {invoiceAddress && status === "pending" && !paying && (
          <button className={styles.retryBtn} onClick={onPay} type="button">
            ⬡ Pay with testnet node
          </button>
        )}

        {paying && (
          <button className={styles.retryBtn} disabled type="button" style={{ opacity: 0.6, cursor: "not-allowed" }}>
            Sending…
          </button>
        )}

        {(isTerminal || error) && status !== "success" && (
          <button className={styles.copyBtn} onClick={onRegenerate} type="button">
...
function SuccessView({ hash, amount, asset, onNewPayment }: {
  hash: HexString;
  amount: number;
  asset: AssetId;
  onNewPayment: () => void;
  ...
  return (
    <div className={styles.successView}>
      ...
      <p className={styles.successAmount}>{amount} {asset}</p>

      <div className={styles.hashBlock}>
        <span className={styles.hashLabel}>Payment hash</span>
        <button className={styles.hashValue} onClick={copy} title="Click to copy">
          <code>{hash.slice(0, 18)}…{hash.slice(-8)}</code>
          <span className={styles.hashCopyHint}>{copied ? "✓" : "copy"}</span>
        </button>
      </div>

      <button className={styles.retryBtn} onClick={onNewPayment} type="button">
        New payment
      </button>
    </div>
  );
...
function StatusBadge({ status, paying }: { status: string; paying?: boolean }) {
  const config: Record<string, { label: string; cls: string }> = {
    idle:       { label: paying ? "Sending payment…" : "Initializing…", cls: styles.statusIdle },
    pending:    { label: paying ? "Sending payment…" : "Waiting for payment",  cls: styles.statusPending },
