...
Peer dependencies — install these alongside:
...
export async function POST(req: NextRequest) {
  if (!NODE_URL) return NextResponse.json({ error: "Not configured" }, { status: 500 });
...
```ts
import {
  ckbToShannonHex, shannonHexToCkb,
  ckbToShannon, shannonToCkb,
  formatAmount,
  toHex, fromHex,
  generatePreimage, isValidPreimage,
  ASSETS, getAsset, isUdtAsset,
} from "fiber-checkout";
